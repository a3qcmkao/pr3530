...
var cloudhost = {
    // 相关配置数据
    dataConfig: {
        cpucount: 7,// 等同于数组 (cpuIndexCountMap.length - 1)  index=0 时 -1 为占位
        ramcount: 12, // 等同于数组 (ramIndexCountMap.length - 1) index=0 时 -1 为占位
        ...
        // CPU 下标对应CPU数量
        cpuIndexCountMap: [-1, 1, 2, 4, 8, 12, 16,32],
    ...
    initCloudHostTab: function () {
        ...
        // 样式修正,（wj--20170505--去掉亚洲数据中心机房云服务器6.5折标识）
        //$("#J_roomsContainerDom [data-value=36]").append('<i class="promo"></i>');

        // 根据type 选中弹性云类型
        var type = WJF.util.getUrlParams('type');
        if (type) {
            this.dataConfig.defaultConfigType = this.dataConfig.defaultConfigTypeMapping[type] || this.dataConfig.defaultConfigType;
        }
            ...
            onSelect: function (value, desc) {
                switch (value) {
                    ...
                    case 'linux_west_slb':
                        ...
                        self.systemSubType.enable();
                        $("#systemSubTypeContainer").html($("#fzjhSubItems").html());
                        self.systemSubType.selectItem();
                        !isInitStatus && self.systemSubType.expandItems();
        ...
        // 根据参数选中指定线路
        var defaultRoomLink;
        var room = WJF.util.getUrlParams('room');
        if (room) {
            // 默认选中的线路 智能多线
            this.serverConfig.room = room;
            $("#J_roomsContainerDom span.active").removeClass('active');
            defaultRoomLink = $("#J_roomsContainerDom [data-value=" + room + "]").addClass('active');
        } else {
            defaultRoomLink = $("#J_roomsContainerDom span.active");
            if(defaultRoomLink.length==0){
                $("#J_roomsContainerDom [data-value=" + this.serverConfig.room + "]").addClass('active');
            }
        }
        var room_ttt = null;
        ...
        var defaultRenewTimeDt ;
        $("#J_renewTimeContainerDom").on('click', 'dt', function (event,param) {
            var target = $(this);
            var value = target.attr('data-value');
            if (value == defaultRenewTimeDt.attr('data-value')) {
                if(!param || param.force!==true)
                return;
            }
        ...
        if (renewTime) {
            ...
            $("#J_renewTimeContainerDom dt.active").removeClass('active');
            defaultRenewTimeDt = $("#J_renewTimeContainerDom dt[data-value=" +renewTime + "]").addClass('active');
            defaultRenewTimeDt.trigger('click',{force:true});
        } else {
            defaultRenewTimeDt = $("#J_renewTimeContainerDom dt.active");
        }
