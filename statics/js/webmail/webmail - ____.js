var mail = {
     ...
     */
    fetchMailItemPrice: function (index) {
        var self = this;
        $("#J_mail_1").removeClass('loading-data').addClass('loading-data');
        $("#J_spaceSize").text($("#J_mailConfigBtns a.link-btn").eq(index).text());
        $("#J_userNum1").val(num);
        $("#J_MailSize1").val(self.priceArray[index]);

        // 清除
        $('#J_priceCount').html('--');
        $('#J_userCount').html('--');
        var t = this.priceArray[index].split(":")[0];
        url = "/services/Mail/customMail/default.asp?act=getneedmoney&userNum=" + num + "&mailsize=" + self.priceArray[index];
        this.fetchItemPriceAjax && this.fetchItemPriceAjax.abort();
        // 获取具体的价格和数量
        this.fetchItemPriceAjax = $.ajax({
            url: url,
            type: 'get',
            success: function (data) {
                var ptemp = data.split(",");
                var ptempstr = ptemp[0].split("^|^");
                // 太快...
                setTimeout(function () {
                    $('#J_priceCount').html(ptempstr[1].match(/(\d+)/)[1]);
                    $('#J_userCount').html(num);
                    $("#J_mail_1").removeClass('loading-data');
                }, 300);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                ...
                layer.alert('抱歉，获取邮箱价格信息失败，请刷新重试');
            }
        });
    },
    /**
